...
const BACKEND_URL = process.env.BACKEND_URL || "http://consumer-administration-dev-alb-46504817.us-east-1.elb.amazonaws.com";
  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      if (error.name === "AbortError") {
        errorCode = "TIMEOUT";
        errorMessage = `La petición excedió el tiempo límite de ${FETCH_TIMEOUT / 1000} segundos`;
      ...
      } else if (error.message.includes("ETIMEDOUT") || error.message.includes("ENETUNREACH")) {
        errorCode = "NETWORK_ERROR";
        errorMessage = "No se puede alcanzar el backend (posiblemente está en una red privada)";
      }
    }
